"""
01_generate_data.py  (MoreComplicatedVersion)
=============================================
Generate dataset for joint estimation of Gaussian spike amplitude AND width.

Each sample:
  - Random Gaussian spike with both A (amplitude) and sigma (width) varied
  - mu (position) remains fixed at the center
  - Simulate wave through it with finite differences
  - Record signal y(t) at far-end sensor

  INPUT : y(t)       — 512 time-series values
  OUTPUT: (A_norm, sigma_norm) — amplitude and width, both normalised to [0,1]

Run:
    python 01_generate_data.py

Output:
    data/dataset.npz   keys: y_signals, params, sigmas, amps,
                              global_mean, global_std
                       params shape: (N, 2)  — [amp_norm, sigma_norm]
...
SEED         = 42
N_SAMPLES    = 5000          # more samples because the problem is harder (2D)
N_POINTS     = 256
SIGNAL_LEN   = 512
DX           = 1e-3
T_TOTAL      = 4e-4
...
MU_FIXED  = N_POINTS // 2   # always at position 128 mm

AMP_MIN   = 40.0             # m/s above water
AMP_MAX   = 200.0
SIGMA_MIN = 5.0              # mm
SIGMA_MAX = 30.0
...
def norm_amp(a):     return (a - AMP_MIN)   / (AMP_MAX   - AMP_MIN)
...
print(f"Generating {N_SAMPLES} samples (mu fixed at {MU_FIXED}mm, A and sigma both vary)...")
y_signals = np.zeros((N_SAMPLES, SIGNAL_LEN), dtype=np.float32)
params    = np.zeros((N_SAMPLES, 2),          dtype=np.float32)  # [amp_norm, sigma_norm]
...
for i in range(N_SAMPLES):
    ...
    sig   = simulate_wave(c)
    y_signals[i]  = sig
    params[i, 0]  = norm_amp(amp)
    params[i, 1]  = norm_sigma(sigma)
    amps[i]       = amp
    sigmas[i]     = sigma

print("Done. Applying global normalisation...")
...
print(f"  params:    {params.shape}  — columns: [amp_norm, sigma_norm]")

# visualise 4 examples
x_mm = np.arange(N_POINTS)
fig, axes = plt.subplots(4, 2, figsize=(11, 10))
fig.suptitle(f"Dataset: Gaussian spike (μ fixed={MU_FIXED}mm, A and σ both vary) → sensor signal",
             fontsize=13)
for row in range(4):
    amp_r = amps[row]; sigma_r = sigmas[row]
    c_plot = make_profile(MU_FIXED, sigma_r, amp_r)
    ...
    axes[row, 0].set_title(f"Sample {row+1}: μ={MU_FIXED}mm  σ={sigma_r:.1f}mm  A={amp_r:.0f}m/s")
